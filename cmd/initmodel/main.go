...
func main() {
	...
	for i := range modelConfigs {
		go func(modelConfig ModelConfig) {
			...
			sCtx := gatewayx.InjectOwnerToContext(context.Background(), owner)
...
func newClients(logger *zap.Logger) (
	...
	modelPublicServiceClient, mclose, err := grpcclientx.NewModelPublicClient(clientx.ServiceConfig{
		Host:        host,
	...
	modelPrivateServiceClient, mclose, err := grpcclientx.NewModelPrivateClient(clientx.ServiceConfig{
		Host:        host,
		PrivatePort: config.Config.Server.PrivatePort,
		PublicPort:  config.Config.Server.PublicPort,
		HTTPS:       config.Config.Server.HTTPS,
