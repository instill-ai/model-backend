...
// NewRayMock returns a mock for ray.Ray
...
func (m *RayMock) MinimockCloseInspect() {
	...
	if !m.CloseMock.invocationsDone() && afterCloseCounter > 0 {
		m.t.Errorf("Expected %d calls to RayMock.Close but found %d calls",
...
func (m *RayMock) MinimockInitInspect() {
	...
	if !m.InitMock.invocationsDone() && afterInitCounter > 0 {
		m.t.Errorf("Expected %d calls to RayMock.Init but found %d calls",
			mm_atomic.LoadUint64(&m.InitMock.expectedInvocations), afterInitCounter)
	}
}
...
// IsRayServerReady implements ray.Ray
	...
	if mmIsRayServerReady.IsRayServerReadyMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmIsRayServerReady.t.Errorf("RayMock.IsRayServerReady got unexpected parameter ctx, want: %#v, got: %#v%s\n", *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmIsRayServerReady.t.Errorf("RayMock.IsRayServerReady got unexpected parameters, want: %#v, got: %#v%s\n", *mm_want, mm_got, minimock.Diff(*mm_want, mm_got))
		}
...
func (m *RayMock) MinimockIsRayServerReadyInspect() {
	for _, e := range m.IsRayServerReadyMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to RayMock.IsRayServerReady with params: %#v", *e.params)
		}
	}
	...
	if m.IsRayServerReadyMock.defaultExpectation != nil && afterIsRayServerReadyCounter < 1 {
		if m.IsRayServerReadyMock.defaultExpectation.params == nil {
			m.t.Error("Expected call to RayMock.IsRayServerReady")
		} else {
			m.t.Errorf("Expected call to RayMock.IsRayServerReady with params: %#v", *m.IsRayServerReadyMock.defaultExpectation.params)
		}
	}
	// if func was set then invocations count should be greater than zero
	...
	if !m.IsRayServerReadyMock.invocationsDone() && afterIsRayServerReadyCounter > 0 {
		m.t.Errorf("Expected %d calls to RayMock.IsRayServerReady but found %d calls",
			mm_atomic.LoadUint64(&m.IsRayServerReadyMock.expectedInvocations), afterIsRayServerReadyCounter)
	}
}
...
// ModelInferRequest implements ray.Ray
func (mmModelInferRequest *RayMock) ModelInferRequest(ctx context.Context, task commonpb.Task, req *modelpb.TriggerNamespaceModelRequest, modelName string, version string) (cp1 *rayserver.CallResponse, err error) {
	...
	if mmModelInferRequest.ModelInferRequestMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmModelInferRequest.t.Errorf("RayMock.ModelInferRequest got unexpected parameter ctx, want: %#v, got: %#v%s\n", *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}

			if mm_want_ptrs.task != nil && !minimock.Equal(*mm_want_ptrs.task, mm_got.task) {
				mmModelInferRequest.t.Errorf("RayMock.ModelInferRequest got unexpected parameter task, want: %#v, got: %#v%s\n", *mm_want_ptrs.task, mm_got.task, minimock.Diff(*mm_want_ptrs.task, mm_got.task))
			}

			if mm_want_ptrs.req != nil && !minimock.Equal(*mm_want_ptrs.req, mm_got.req) {
				mmModelInferRequest.t.Errorf("RayMock.ModelInferRequest got unexpected parameter req, want: %#v, got: %#v%s\n", *mm_want_ptrs.req, mm_got.req, minimock.Diff(*mm_want_ptrs.req, mm_got.req))
			}

			if mm_want_ptrs.modelName != nil && !minimock.Equal(*mm_want_ptrs.modelName, mm_got.modelName) {
				mmModelInferRequest.t.Errorf("RayMock.ModelInferRequest got unexpected parameter modelName, want: %#v, got: %#v%s\n", *mm_want_ptrs.modelName, mm_got.modelName, minimock.Diff(*mm_want_ptrs.modelName, mm_got.modelName))
			}

			if mm_want_ptrs.version != nil && !minimock.Equal(*mm_want_ptrs.version, mm_got.version) {
				mmModelInferRequest.t.Errorf("RayMock.ModelInferRequest got unexpected parameter version, want: %#v, got: %#v%s\n", *mm_want_ptrs.version, mm_got.version, minimock.Diff(*mm_want_ptrs.version, mm_got.version))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmModelInferRequest.t.Errorf("RayMock.ModelInferRequest got unexpected parameters, want: %#v, got: %#v%s\n", *mm_want, mm_got, minimock.Diff(*mm_want, mm_got))
		}
...
func (m *RayMock) MinimockModelInferRequestInspect() {
	for _, e := range m.ModelInferRequestMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to RayMock.ModelInferRequest with params: %#v", *e.params)
		}
	}
	...
	if m.ModelInferRequestMock.defaultExpectation != nil && afterModelInferRequestCounter < 1 {
		if m.ModelInferRequestMock.defaultExpectation.params == nil {
			m.t.Error("Expected call to RayMock.ModelInferRequest")
		} else {
			m.t.Errorf("Expected call to RayMock.ModelInferRequest with params: %#v", *m.ModelInferRequestMock.defaultExpectation.params)
		}
	}
	// if func was set then invocations count should be greater than zero
	...
	if !m.ModelInferRequestMock.invocationsDone() && afterModelInferRequestCounter > 0 {
		m.t.Errorf("Expected %d calls to RayMock.ModelInferRequest but found %d calls",
			mm_atomic.LoadUint64(&m.ModelInferRequestMock.expectedInvocations), afterModelInferRequestCounter)
	}
}
...
func (mmModelReady *RayMock) ModelReady(ctx context.Context, modelName string, version string) (sp1 *modelpb.State, s1 string, i1 int, err error) {
	...
	if mmModelReady.ModelReadyMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmModelReady.t.Errorf("RayMock.ModelReady got unexpected parameter ctx, want: %#v, got: %#v%s\n", *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}

			if mm_want_ptrs.modelName != nil && !minimock.Equal(*mm_want_ptrs.modelName, mm_got.modelName) {
				mmModelReady.t.Errorf("RayMock.ModelReady got unexpected parameter modelName, want: %#v, got: %#v%s\n", *mm_want_ptrs.modelName, mm_got.modelName, minimock.Diff(*mm_want_ptrs.modelName, mm_got.modelName))
			}

			if mm_want_ptrs.version != nil && !minimock.Equal(*mm_want_ptrs.version, mm_got.version) {
				mmModelReady.t.Errorf("RayMock.ModelReady got unexpected parameter version, want: %#v, got: %#v%s\n", *mm_want_ptrs.version, mm_got.version, minimock.Diff(*mm_want_ptrs.version, mm_got.version))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmModelReady.t.Errorf("RayMock.ModelReady got unexpected parameters, want: %#v, got: %#v%s\n", *mm_want, mm_got, minimock.Diff(*mm_want, mm_got))
		}
...
func (m *RayMock) MinimockModelReadyInspect() {
	for _, e := range m.ModelReadyMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to RayMock.ModelReady with params: %#v", *e.params)
		}
	}
	...
	if m.ModelReadyMock.defaultExpectation != nil && afterModelReadyCounter < 1 {
		if m.ModelReadyMock.defaultExpectation.params == nil {
			m.t.Error("Expected call to RayMock.ModelReady")
		} else {
			m.t.Errorf("Expected call to RayMock.ModelReady with params: %#v", *m.ModelReadyMock.defaultExpectation.params)
		}
	}
	// if func was set then invocations count should be greater than zero
	...
	if !m.ModelReadyMock.invocationsDone() && afterModelReadyCounter > 0 {
		m.t.Errorf("Expected %d calls to RayMock.ModelReady but found %d calls",
			mm_atomic.LoadUint64(&m.ModelReadyMock.expectedInvocations), afterModelReadyCounter)
	}
}
...
func (mmUpdateContainerizedModel *mRayMockUpdateContainerizedModel) Expect(ctx context.Context, modelName string, userID string, imageName string, version string, hardware string, action mm_ray.Action, scalingConfig []string, numOfGPU int) *mRayMockUpdateContainerizedModel {
	...
	mmUpdateContainerizedModel.defaultExpectation.params = &RayMockUpdateContainerizedModelParams{ctx, modelName, userID, imageName, version, hardware, action, scalingConfig, numOfGPU}
...
func (mmUpdateContainerizedModel *mRayMockUpdateContainerizedModel) When(ctx context.Context, modelName string, userID string, imageName string, version string, hardware string, action mm_ray.Action, scalingConfig []string, numOfGPU int) *RayMockUpdateContainerizedModelExpectation {
	...
	expectation := &RayMockUpdateContainerizedModelExpectation{
		...
		params: &RayMockUpdateContainerizedModelParams{ctx, modelName, userID, imageName, version, hardware, action, scalingConfig, numOfGPU},
	}
...
func (mmUpdateContainerizedModel *RayMock) UpdateContainerizedModel(ctx context.Context, modelName string, userID string, imageName string, version string, hardware string, action mm_ray.Action, scalingConfig []string, numOfGPU int) (err error) {
	...
	if mmUpdateContainerizedModel.inspectFuncUpdateContainerizedModel != nil {
		mmUpdateContainerizedModel.inspectFuncUpdateContainerizedModel(ctx, modelName, userID, imageName, version, hardware, action, scalingConfig, numOfGPU)
	}

	mm_params := RayMockUpdateContainerizedModelParams{ctx, modelName, userID, imageName, version, hardware, action, scalingConfig, numOfGPU}
	...
	if mmUpdateContainerizedModel.UpdateContainerizedModelMock.defaultExpectation != nil {
		...
		mm_got := RayMockUpdateContainerizedModelParams{ctx, modelName, userID, imageName, version, hardware, action, scalingConfig, numOfGPU}

		if mm_want_ptrs != nil {

			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmUpdateContainerizedModel.t.Errorf("RayMock.UpdateContainerizedModel got unexpected parameter ctx, want: %#v, got: %#v%s\n", *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}

			if mm_want_ptrs.modelName != nil && !minimock.Equal(*mm_want_ptrs.modelName, mm_got.modelName) {
				mmUpdateContainerizedModel.t.Errorf("RayMock.UpdateContainerizedModel got unexpected parameter modelName, want: %#v, got: %#v%s\n", *mm_want_ptrs.modelName, mm_got.modelName, minimock.Diff(*mm_want_ptrs.modelName, mm_got.modelName))
			}

			if mm_want_ptrs.userID != nil && !minimock.Equal(*mm_want_ptrs.userID, mm_got.userID) {
				mmUpdateContainerizedModel.t.Errorf("RayMock.UpdateContainerizedModel got unexpected parameter userID, want: %#v, got: %#v%s\n", *mm_want_ptrs.userID, mm_got.userID, minimock.Diff(*mm_want_ptrs.userID, mm_got.userID))
			}

			if mm_want_ptrs.imageName != nil && !minimock.Equal(*mm_want_ptrs.imageName, mm_got.imageName) {
				mmUpdateContainerizedModel.t.Errorf("RayMock.UpdateContainerizedModel got unexpected parameter imageName, want: %#v, got: %#v%s\n", *mm_want_ptrs.imageName, mm_got.imageName, minimock.Diff(*mm_want_ptrs.imageName, mm_got.imageName))
			}

			if mm_want_ptrs.version != nil && !minimock.Equal(*mm_want_ptrs.version, mm_got.version) {
				mmUpdateContainerizedModel.t.Errorf("RayMock.UpdateContainerizedModel got unexpected parameter version, want: %#v, got: %#v%s\n", *mm_want_ptrs.version, mm_got.version, minimock.Diff(*mm_want_ptrs.version, mm_got.version))
			}

			if mm_want_ptrs.hardware != nil && !minimock.Equal(*mm_want_ptrs.hardware, mm_got.hardware) {
				mmUpdateContainerizedModel.t.Errorf("RayMock.UpdateContainerizedModel got unexpected parameter hardware, want: %#v, got: %#v%s\n", *mm_want_ptrs.hardware, mm_got.hardware, minimock.Diff(*mm_want_ptrs.hardware, mm_got.hardware))
			}

			if mm_want_ptrs.action != nil && !minimock.Equal(*mm_want_ptrs.action, mm_got.action) {
				mmUpdateContainerizedModel.t.Errorf("RayMock.UpdateContainerizedModel got unexpected parameter action, want: %#v, got: %#v%s\n", *mm_want_ptrs.action, mm_got.action, minimock.Diff(*mm_want_ptrs.action, mm_got.action))
			}

			if mm_want_ptrs.scalingConfig != nil && !minimock.Equal(*mm_want_ptrs.scalingConfig, mm_got.scalingConfig) {
				mmUpdateContainerizedModel.t.Errorf("RayMock.UpdateContainerizedModel got unexpected parameter scalingConfig, want: %#v, got: %#v%s\n", *mm_want_ptrs.scalingConfig, mm_got.scalingConfig, minimock.Diff(*mm_want_ptrs.scalingConfig, mm_got.scalingConfig))
			}

			if mm_want_ptrs.numOfGPU != nil && !minimock.Equal(*mm_want_ptrs.numOfGPU, mm_got.numOfGPU) {
				mmUpdateContainerizedModel.t.Errorf("RayMock.UpdateContainerizedModel got unexpected parameter numOfGPU, want: %#v, got: %#v%s\n", *mm_want_ptrs.numOfGPU, mm_got.numOfGPU, minimock.Diff(*mm_want_ptrs.numOfGPU, mm_got.numOfGPU))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmUpdateContainerizedModel.t.Errorf("RayMock.UpdateContainerizedModel got unexpected parameters, want: %#v, got: %#v%s\n", *mm_want, mm_got, minimock.Diff(*mm_want, mm_got))
		}
	...
	if mmUpdateContainerizedModel.funcUpdateContainerizedModel != nil {
		return mmUpdateContainerizedModel.funcUpdateContainerizedModel(ctx, modelName, userID, imageName, version, hardware, action, scalingConfig, numOfGPU)
	}
	mmUpdateContainerizedModel.t.Fatalf("Unexpected call to RayMock.UpdateContainerizedModel. %v %v %v %v %v %v %v %v %v", ctx, modelName, userID, imageName, version, hardware, action, scalingConfig, numOfGPU)
	return
}
...
func (m *RayMock) MinimockUpdateContainerizedModelInspect() {
	for _, e := range m.UpdateContainerizedModelMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to RayMock.UpdateContainerizedModel with params: %#v", *e.params)
		}
	}
	...
	if m.UpdateContainerizedModelMock.defaultExpectation != nil && afterUpdateContainerizedModelCounter < 1 {
		if m.UpdateContainerizedModelMock.defaultExpectation.params == nil {
			m.t.Error("Expected call to RayMock.UpdateContainerizedModel")
		} else {
			m.t.Errorf("Expected call to RayMock.UpdateContainerizedModel with params: %#v", *m.UpdateContainerizedModelMock.defaultExpectation.params)
		}
	}
	// if func was set then invocations count should be greater than zero
	...
	if !m.UpdateContainerizedModelMock.invocationsDone() && afterUpdateContainerizedModelCounter > 0 {
		m.t.Errorf("Expected %d calls to RayMock.UpdateContainerizedModel but found %d calls",
			mm_atomic.LoadUint64(&m.UpdateContainerizedModelMock.expectedInvocations), afterUpdateContainerizedModelCounter)
	}
}
