-- Migration: Unify model ID to AIP standard (hash-based canonical ID)
...
CREATE OR REPLACE FUNCTION generate_model_canonical_id(uid UUID)
RETURNS TEXT AS $$
DECLARE
    hash_bytes BYTEA;
    result TEXT := '';
...
UPDATE model
SET id = generate_model_canonical_id(uid)
WHERE id IS NULL OR id = '' OR id NOT LIKE 'mod-%';

-- Step 5: Add unique constraint on (owner, id) for canonical IDs
ALTER TABLE model DROP CONSTRAINT IF EXISTS model_owner_id_unique;
CREATE UNIQUE INDEX IF NOT EXISTS idx_model_owner_id_unique ON model(owner, id);

-- Step 6: Clean up the helper function
DROP FUNCTION IF EXISTS generate_model_canonical_id(UUID);
